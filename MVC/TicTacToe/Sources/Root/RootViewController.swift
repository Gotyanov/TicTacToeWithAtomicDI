...
class RootViewController: UIViewController {
    ...
    private let loggedOutBuilder: () -> UIViewController
    private let loggedInBuilder: () -> UIViewController
    private var playersStreamDisposable: Disposable?

    init(playersStream: PlayersStream, loggedOutBuilder: @escaping () -> UIViewController, loggedInBuilder: @escaping () -> UIViewController) {
    ...
    private func updateChildViewController() {
        ...
        playersStreamDisposable = playersStream.names
            ...
            .subscribe(onNext: { [weak self] (state: RootChildStates) in
                ...
                switch state {
                case .loggedIn:
                    strongSelf.present(viewController: strongSelf.loggedInBuilder())
                case .loggedOut:
                    strongSelf.present(viewController: strongSelf.loggedOutBuilder())
                }
            })
    }
